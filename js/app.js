...
// --- SKIN CACHE BUSTING ---
// Generates a fresh timestamp per page load so skin/username changes show immediately
let SKIN_CACHE_BUSTER = Date.now();
function skinUrl(url) {
  return url + (url.includes('?') ? '&' : '?') + '_cb=' + SKIN_CACHE_BUSTER;
}
...
async function startBackgroundSync() {
  ...
  for (let i = 0; i < toCheck.length; i++) {
    ...
    try {
      ...
      console.warn(`Failed to sync player ${player.username}:`, err);
    }
  }
...
// Load players & testers from database.json, LocalStorage, or fall back to INITIAL_PLAYERS & INITIAL_TESTERS
async function loadDatabase() {
  let loadedFromJSON = false;

  // 1. Primary: Try to fetch authoritative js/database.json directly
  try {
    let res = await fetch(`js/database.json?_cb=${Date.now()}`, { cache: 'no-store' });
    if (!res.ok) {
      res = await fetch(`database.json?_cb=${Date.now()}`, { cache: 'no-store' });
    }
    if (res.ok) {
      const data = await res.json();
      if (data && Array.isArray(data.players) && data.players.length > 0) {
        ...
        loadedFromJSON = true;
        try { window.__mctiers_data_source = 'js/database.json'; } catch (e) {}
        console.log(`Successfully loaded ${players.length} players and ${testers.length} testers from js/database.json`);
      }
    }
  } catch (err) {
    console.log("Direct js/database.json fetch not available, checking fallbacks...", err);
  }
...
function syncToServer() {
  ...
  try {
    ...
    .catch((err) => {
      // Quietly log — do not trigger file save prompts or toasts on normal page views
      console.log('Local Node server not active for POST sync.');
    });
  } catch (err) {
    console.log('Sync failed before request was sent.', err);
  }
}async function saveDatabaseFileToDisk({ silent = false } = {}) {
  const jsonContent = syncDatabaseFile();
  ...
  if (databaseFileHandle) {
    try {
      ...
      if (!silent) {
        showToast('Saved database.json');
      }
      return true;
    } catch (err) {
      console.warn('Reusing saved file handle failed, prompting again.', err);
  ...
  if (window.showSaveFilePicker) {
    try {
      const handle = await window.showSaveFilePicker({
        ...
        types: [{
          description: 'JSON Database files',
  ...
  if (!silent) {
    showToast('Downloaded database.json');
  }
  return true;
}

// Auto-sync the current state back to JSON & database.js format
...
`;
  ...
  localStorage.setItem("jordan_mctiers_sync_export", jsonContent);
  return jsonContent;
}
...
function updateLiveStats() {
  ...
  if (editorCountSpan) animateCount(editorCountSpan, players.length);

  const editorTestersSpan = document.getElementById("editor-testers-count");
  if (editorTestersSpan) animateCount(editorTestersSpan, testers.length);
}
...
// --- PLAYER ACHIEVEMENT TITLES (Based on Total Points) ---
function getPlayerTitle(totalPts) {
  const pts = typeof totalPts === "number" ? totalPts : 0;
  if (pts >= 400) {
    return { title: "Combat Grandmaster", tierClass: "title-grandmaster", color: "#f5c518", iconSvg: '<path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>' };
  }
  if (pts >= 250) {
    return { title: "Combat Master", tierClass: "title-master", color: "#f0b232", iconSvg: '<polygon points="12 2 19 12 12 22 5 12"/>' };
  }
  if (pts >= 100) {
    return { title: "Combat Ace", tierClass: "title-ace", color: "#ff5277", iconSvg: '<polygon points="12 2 19 12 12 22 5 12"/>' };
  }
  if (pts >= 50) {
    return { title: "Combat Specialist", tierClass: "title-specialist", color: "#b388ff", iconSvg: '<polygon points="12 2 19 12 12 22 5 12"/>' };
  }
  if (pts >= 20) {
    return { title: "Combat Cadet", tierClass: "title-cadet", color: "#8c9eff", iconSvg: '<polygon points="12 2 19 12 12 22 5 12"/>' };
  }
  if (pts >= 10) {
    return { title: "Combat Novice", tierClass: "title-novice", color: "#9cb1c6", iconSvg: '<polygon points="12 2 19 12 12 22 5 12"/>' };
  }
  return { title: "Rookie", tierClass: "title-rookie", color: "#7a8c9e", iconSvg: '<polygon points="12 2 19 12 12 22 5 12"/>' };
}

// --- PLAYER RETIRED STATUS HELPER ---
function isPlayerRetiredInGamemode(player, gmId) {
  if (!player || !player.retired) return false;
  if (Array.isArray(player.retired)) {
    return player.retired.includes(gmId);
  }
  return !!player.retired[gmId];
}
...
function renderTierList() {
  ...
  if (activeGamemode === "overall") {
    ...
    // 1. Calculate true global overall rank for ALL players based on overall points
    const globalOverallRankMap = new Map();
    players
      .map(p => ({ player: p, pts: calculateTotalPoints(p.tiers) }))
      .filter(({ pts }) => pts > 0)
      .sort((a, b) => b.pts - a.pts)
      .forEach(({ player }, idx) => {
        globalOverallRankMap.set(player.username, idx + 1);
      });

    // 2. Map filtered players with their total points and true global rank
    const ranked = filteredPlayers
      .map(p => ({ 
        player: p, 
        pts: calculateTotalPoints(p.tiers),
        globalRank: globalOverallRankMap.get(p.username) || 0
      }))
      .filter(({ pts }) => pts > 0)
      .sort((a, b) => b.pts - a.pts);
    ...
    ranked.forEach(({ player, pts, globalRank }, index) => {
      const trueRank = globalRank > 0 ? globalRank : (index + 1);
      const row = createOverallRankRow(player, pts, trueRank);
    ...
    filteredPlayers.forEach(player => {
      // Exclude players from the specific gamemode tierlist if they are retired in it
      if (isPlayerRetiredInGamemode(player, activeGamemode)) {
        return;
      }
...
function createOverallRankRow(player, pts, rank) {
  ...
  const titleInfo = getPlayerTitle(pts);
  ...
  const gamemodeChips = INITIAL_GAMEMODES
    ...
    .map(gm => {
      ...
      const isRetired = isPlayerRetiredInGamemode(player, gm.id);
      const isTested = t !== "None";
      const tierPoints = TIER_POINTS[t] || 0;
      const titleText = !isTested ? `${gm.name}: Untested` : (isRetired ? `Retired ${t}` : `${gm.name}: ${t}`);
      const ptsText = !isTested ? '0 points' : `${tierPoints} ${tierPoints === 1 ? 'point' : 'points'}`;
      
      if (!isTested) {
        return `
          <div class="lb-hud-item is-untested">
            <div class="lb-hud-icon dashed"></div>
            <span class="lb-hud-val badge-none">-</span>
            <div class="lb-tier-tooltip">
              <div class="lb-tier-tooltip-title">${titleText}</div>
              <div class="lb-tier-tooltip-pts">${ptsText}</div>
            </div>
          </div>
        `;
      }
      ...
      return `
        <div class="lb-hud-item ${isRetired ? 'is-retired' : ''}">
          <div class="lb-hud-icon">${gm.icon}</div>
          <span class="lb-hud-val badge-${t.toLowerCase()}">${t}</span>
          <div class="lb-tier-tooltip">
            <div class="lb-tier-tooltip-title">${titleText}</div>
            <div class="lb-tier-tooltip-pts">${ptsText}</div>
          </div>
  ...
  row.innerHTML = `
    ...
    <div class="lb-player-cell no-avatar">
      <div class="lb-player-info">
        ...
        <span class="lb-player-title ${titleInfo.tierClass}">
          <svg class="lb-title-icon" viewBox="0 0 24 24" fill="currentColor">
            ${titleInfo.iconSvg}
          </svg>
          <span class="lb-title-text">${titleInfo.title}</span>
          <span class="lb-points">(${pts} points)</span>
        </span>
      </div>
    </div>
    <div class="lb-region-cell">
      <span class="lb-region-badge">${(player.region || 'EU').toUpperCase()}</span>
    </div>
    <div class="lb-tiers-cell">
      <div class="lb-hud-strip">
        ${gamemodeChips || '<span style="font-size:0.75rem;color:var(--text-muted);padding:8px">No active tiers</span>'}
      </div>
    </div>
  `;
...
function createPlayerCard(player) {
  ...
  const skinId = skinIdentifier(player);
  const avatarUrl = skinUrl(`https://visage.surgeplay.com/bust/128/${encodeURIComponent(skinId)}`);
  ...
  let detailsHTML = `
    ...
    <div class="player-info">
      ...
      <span class="player-subtext">
        ...
        <span class="player-city-dot">â€¢</span>
...
function openPlayerModal(player) {
  ...
  if (namemcLink) {
    if (player.uuid === 'cracked') {
      namemcLink.classList.add("hidden");
    } else {
      namemcLink.classList.remove("hidden");
...
function openInfoModal() {
  ...
  if (testers.length === 0) {
    ...
    testers.forEach(tester => {
      ...
      const headUrl = skinUrl(`https://visage.surgeplay.com/bust/64/${encodeURIComponent(cleanTester)}`);
      const isOnline = tester.online === true;

      card.innerHTML = `
        <div class="tester-avatar-wrapper">
          <img class="tester-avatar" src="${headUrl}" alt="${tester.name}" onerror="handleBustError(this, 64)">
        </div>
...
function openEditorModal() {
  ...
  if (assignGrid) {
    ...
    INITIAL_GAMEMODES.filter(gm => gm.id !== "overall").forEach(gm => {
      ...
      row.innerHTML = `
        <label class="form-tier-name" for="assign-${gm.id}">${gm.name}</label>
        <div class="form-tier-controls">
          <select id="assign-${gm.id}" name="${gm.id}" class="editor-gm-select">
            <option value="None">None</option>
            ${TIER_ORDER.map(t => `<option value="${t}">${t}</option>`).join("")}
          </select>
          <label class="retire-toggle-label" title="Mark as retired in ${gm.name}">
            <input type="checkbox" id="retire-${gm.id}" class="retire-gm-checkbox">
            <span>Retired</span>
          </label>
        </div>
      `;
...
function resetPlayerForm() {
  ...
  if (regionEl) regionEl.value = "EU";

  INITIAL_GAMEMODES.filter(gm => gm.id !== "overall").forEach(gm => {
    const retireCheck = document.getElementById(`retire-${gm.id}`);
    if (retireCheck) retireCheck.checked = false;
  });
}
...
function startPlayerEdit(index) {
  ...
  INITIAL_GAMEMODES.filter(gm => gm.id !== "overall").forEach(gm => {
    ...
    const retireCheck = document.getElementById(`retire-${gm.id}`);
    if (retireCheck) {
      retireCheck.checked = isPlayerRetiredInGamemode(player, gm.id);
    }
  });
...
async function savePlayer(event) {
  ...
  try {
    ...
    INITIAL_GAMEMODES.filter(gm => gm.id !== "overall").forEach(gm => {
      ...
      const retireCheck = document.getElementById(`retire-${gm.id}`);
      const tierVal = select ? select.value : "None";
      newTiers[gm.id] = tierVal;
      if (retireCheck && retireCheck.checked && tierVal !== "None") {
        newRetired[gm.id] = true;
      }
    });
    ...
    try {
      ...
      console.warn("Failed to fetch UUID on save:", e);
    }
...
function renderEditorTestersList() {
  ...
  testers.forEach((tester, index) => {
    ...
    const avatarUrl = skinUrl(`https://visage.surgeplay.com/bust/48/${encodeURIComponent(cleanTesterAvatar)}`);
    const isOnline = tester.online === true;

    item.innerHTML = `
      <img class="editor-player-avatar" src="${avatarUrl}" alt="${tester.name}" onerror="handleBustError(this, 48)">
...
function saveTester(event) {
  ...
  if (index === -1) {
    ...
    existingTester.name = name;
    existingTester.avatar = name;
...
function exportDatabase() {
  ...
  showToast("âœ… Exported mctiers_database.json - You can import this back anytime!");
}
...
function showImportConfirmation(data) {
  ...
  if (data.testers && data.testers.length > 0) {
    data.testers.forEach(t => {
      previewHTML += `â€¢ ${t.name} (${t.role})<br>`;
    });
  } else {
    previewHTML += `(none)<br>`;
  }
...
function confirmImport() {
  ...
  try {
    ...
    showToast("âœ… Database imported and saved successfully!");
  } catch (err) {
  ...
  try {
    if (typeof Lenis !== "undefined") {
      lenisInstance = new Lenis({
        ...
        prevent: (node) => {
          return node && (
            node.closest('.modal-card, .modal-backdrop, .info-modal-body, .modal-body, .info-tab-content, [data-lenis-prevent]') !== null
          );
        }
      });
      ...
      // Observe modals to prevent background window scroll while modal is open
      const observer = new MutationObserver(() => syncModalScrollState());
      document.querySelectorAll(".modal-backdrop").forEach(m => {
        observer.observe(m, { attributes: true, attributeFilter: ["class"] });
      });
    }
  } catch (err) {
    console.warn("Lenis smooth scrolling not initialized, using native smooth scroll:", err);
  }
}
...
function setupEventListeners() {
  ...
  // Interactive mouse ambient spotlight (global)
  window.addEventListener("pointermove", (e) => {
    document.documentElement.style.setProperty("--mouse-x", `${e.clientX}px`);
    document.documentElement.style.setProperty("--mouse-y", `${e.clientY}px`);
  }, { passive: true });

  // Interactive 3D tilt & specular shine on cards / rows (Instant zero-delay response)
  const updateCardCoordinates = (e) => {
    const card = e.target.closest(".overall-rank-row, .player-card, .column-player-card, .btn, .search-wrapper");
    if (card) {
      ...
      // Subtle & refined 3D tilt
      if (card.classList.contains("overall-rank-row") || card.classList.contains("player-card") || card.classList.contains("column-player-card")) {
        const centerX = rect.width / 2;
        const centerY = rect.height / 2;
        const rotateX = ((y - centerY) / centerY) * -0.55;
        const rotateY = ((x - centerX) / centerX) * 0.65;
  ...
  document.addEventListener("pointermove", updateCardCoordinates, { passive: true });
  document.addEventListener("pointerover", updateCardCoordinates, { passive: true });

  // Reset 3D tilt when leaving card
  document.addEventListener("pointerout", (e) => {
    const card = e.target.closest(".overall-rank-row, .player-card, .column-player-card");
  ...
  window.addEventListener("keydown", (e) => {
    if (e.key === "/" && document.activeElement !== mainSearch && !e.ctrlKey && !e.metaKey && !e.altKey) {
  ...
  mainSearch.addEventListener("input", (e) => {
    ...
    if (val.trim()) {
      ...
      // If user completely cleared the input box, reset the tierlist
      if (searchQuery !== "") {
        searchQuery = "";
        renderTierList();
      }
  ...
  mainSearch.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      ...
      if (query) {
        // Find matched player (exact username match first, then nickname, then partial)
        const qLower = query.toLowerCase();
        const matched = players.find(p => p.username.toLowerCase() === qLower)
          || players.find(p => p.nickname && p.nickname.toLowerCase() === qLower)
          || players.find(p => p.username.toLowerCase().includes(qLower));
    ...
    const verifyAndCapitalizeUsername = async () => {
      ...
      try {
        ...
        if (data.success && data.data && data.data.player) {
          ...
          helpText.innerHTML = `
            <div style="display:flex; align-items:center; gap:8px; margin-top:8px; color: var(--warning-color); font-size: 0.72rem;">
              <img src="${skinUrl(`https://visage.surgeplay.com/bust/24/c06f89064c8a49119c29ea1dbd1aab82`)}" style="border-radius:4px; width:16px; height:16px; image-rendering:pixelated;" alt="Steve">
              <span>Cracked account. Saving will use standard Steve model.</span>
            </div>
          `;
        }
      } catch (err) {
        ...
        helpText.innerHTML = `
          <div style="display:flex; align-items:center; gap:8px; margin-top:8px; color: var(--warning-color); font-size: 0.72rem;">
            <img src="${skinUrl(`https://visage.surgeplay.com/bust/24/c06f89064c8a49119c29ea1dbd1aab82`)}" style="border-radius:4px; width:16px; height:16px; image-rendering:pixelated;" alt="Steve">
            <span>Offline validation (Cracked account assumed, using Steve model).</span>
          </div>
        `;
      }
    };
