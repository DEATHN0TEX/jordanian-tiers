...
const SKIN_CACHE_BUSTER = Date.now();
...
function syncToServer() {
  ...
  try {
    ...
    .catch(async (err) => {
      console.warn('âš ï¸ Could not sync to the Node server. Trying to save the database file directly instead.', err);
    ...
    console.warn('âš ï¸ Sync failed before request was sent.', err);
  }
}
...
async function saveDatabaseFileToDisk({ silent = false } = {}) {
  ...
  if (databaseFileHandle) {
    try {
      ...
      console.warn('âš ï¸ Reusing saved file handle failed, prompting again.', err);
  ...
  if (window.showSaveFilePicker) {
    try {
      ...
      if (err && err.name !== 'AbortError') {
        console.warn('âš ï¸ File picker save failed, falling back to download.', err);
      }
    }
  }
...
function openPlayerModal(player) {
  ...
  const sources = [
    ...
    `https://crafatar.com/renders/body/${encodeURIComponent(cleanModalName)}?size=512&default=c06f89064c8a49119c29ea1dbd1aab82`,
    `https://minotar.net/armor/body/${encodeURIComponent(cleanModalName)}/512.png`,
    `https://visage.surgeplay.com/body/512/c06f89064c8a49119c29ea1dbd1aab82`,
    ...
    player.history.forEach(hist => {
      ...
      timelineItem.innerHTML = `
        ...
        <div class="timeline-content">
          <span class="timeline-meta">${hist.date} â€¢ <strong>${hist.gamemode}</strong></span>
  ...
  if (btnEditorToggle) {
    ...
    const verifyAndCapitalizeUsername = async () => {
      ...
      try {
        ...
        if (data.success && data.data && data.data.player) {
          ...
          helpText.innerHTML = `
            <div style="display:flex; align-items:center; gap:8px; margin-top:8px; color: var(--success-color); font-size: 0.72rem;">
              <img src="https://crafatar.com/renders/bust/${encodeURIComponent(correctName)}?size=24&default=c06f89064c8a49119c29ea1dbd1aab82" style="border-radius:4px; width:16px; height:16px; image-rendering:pixelated;" alt="${correctName}" onerror="handleBustError(this, 24)">
        ...
        helpText.innerHTML = `
          <div style="display:flex; align-items:center; gap:8px; margin-top:8px; color: var(--text-muted); font-size: 0.72rem;">
            <img src="https://crafatar.com/renders/bust/${encodeURIComponent(username)}?size=24&default=c06f89064c8a49119c29ea1dbd1aab82" style="border-radius:4px; width:16px; height:16px; image-rendering:pixelated;" alt="${username}" onerror="handleBustError(this, 24)">
